...
game_state.main.prototype = {
    ...
    preload: function () {
        ...
        this.game.load.image('other', 'assets/other.png');
        this.users = [];
    },
    move: function (vx, vy) {
        var me = this.me;
    ...
    updateUser: function (data, sprite) {
        if (data.userId) {
            ...
            if (!user) {
                user = this.game.add.sprite(data.x, data.y, sprite || 'other');
                this.users[data.userId] = user;
                this.users.push(user);
            }
            user.x = data.x;
            user.y = data.y;
            user.body.velocity.x = data.vx;
            user.body.velocity.y = data.vy;
            return user;
        }
        return null;
    },
    destory: function (userId) {
        if (this.users[userId]) {
            ...
            users.splice(users.indexOf(users[userId]), 1);
            users[userId].destroy();
            delete users[userId];
        }
    },
    destoryAll: function (userId) {
        var users = this.users;
        while (users.length > 0) {
            this.destory(users.pop().userId);
        }
        if (this.me) {
            this.me = null;
        }
    ...
    createSelf: function (data) {
        this.destoryAll();
        var me = this.updateUser(data, 'me');
        if (me && !this.me) {
    ...
    update: function () {
        ...
        for (var i = 0; i < this.users.length; i++) {
            ...
            if (user.y < 0) {
                user.y = 300;
                console.log('reset user y:300');
            }
        }
        // If the bird overlap any pipes, call 'restart_game'
        // this.game.physics.overlap(this.me, this.pipes, this.restart_game, null, this);
    },
...
var main_state = game.state.add('main', game_state.main);
